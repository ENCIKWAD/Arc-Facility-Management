    ...
    <div class="background-1 pa-8">
        ...
        <v-card v-else class="pa-8 flex-card">
            ...
            <div class="ml-15 full">
                <div class="flex-text">
                <v-card-title style="font-size: 30px;" class="mt-1">{{ facility.title }}</v-card-title>
                ...
                <div class="flex-text">
                    <v-card-title style="color:green" >{{ formatCurrency(facility.price) }}/hr</v-card-title>
                    <div>
            <v-icon size="45">mdi-account</v-icon>
            <p>Min: {{ facility.minCap }}</p>
        </div>
    </div>
    <v-card-title v-if="!facility.available" style="color: red;" >{{ formatAvailability(facility.available) }}</v-card-title>
    <v-card-title v-if="facility.available" style="color: green" >{{ formatAvailability(facility.available) }}</v-card-title>
                <div class="flex-btn">
                    <v-btn v-if="user.role != 'owner'" :disabled="!facility.available" color="#5F3DAC" width="350" height="50" ><v-icon class="mr-2" size="30" >mdi-book</v-icon>Lease Facility</v-btn>
                    <v-btn :to="{name: `${user.role}Home`}" color="#514D5A" width="350" height="50"><v-icon class="mr-2" size="30" >mdi-arrow-left-circle</v-icon>Go Back</v-btn>
