<template>
    <div class="background-1 pa-4">
      ...
      :userRole="user.role"
    ></navBar>
        <v-container :class="{'margin' : this.$route.query.message}" class="d-flex justify-center align-center inbox-container">
            ...
            <div class="main-container d-flex">
                ...
                <div v-if="selectedUser" class="right-container">
                    ...
                    <v-card-title style="font-size: 25px;" >Subject: {{ this.selectedInbox.title }}</v-card-title>
                    <v-card class="report-container pa-4">
                        <div v-if="this.selectedFacility != null" class="left-card">
                            <v-img cover height="250" width="360" :src="this.selectedFacility.image"></v-img>
                        </div>
                        <div v-if="this.user.role === 'admin'" class="right-card">
                            <v-card-title v-if="this.selectedFacility != null" >{{ this.selectedFacility.title }}</v-card-title>
                            <v-card-text v-if="this.selectedInbox.message" >{{ this.selectedInbox.message }}</v-card-text>
                            <v-card-text v-if="this.selectedInbox.tenantId != null">Tenant ID: {{ this.selectedInbox.tenantId }}</v-card-text>
                            <v-card-text>Date: {{ formatDate(this.selectedInbox.date) }}</v-card-text>
                            <div class="report-delete">
                                <v-btn :to="{ name: 'deleteReport', params: { id: this.selectedInbox._id } }" variant="plain"><v-icon size=30 color="red">mdi-delete</v-icon></v-btn>
    
                            </div>
                        </div>
                        <div v-if="this.user.role === 'owner' && this.selectedFacility != null" class="right-card">
                            <v-card-title  >{{ this.selectedFacility.title }}</v-card-title>
                            <v-card-text>Date: {{ formatDateOnly(this.selectedInbox.startDate) }}</v-card-text>
                            <v-card-text>Duration: {{ duration }}</v-card-text>
                            <v-card-text>Capacity: {{ this.selectedInbox.minCap }}</v-card-text>
                            <v-card-text>Location: {{ this.selectedFacility.location }}</v-card-text>
                            <v-card-text style="color: green;">{{ formatCurrency(this.selectedInbox.price) }}/hr</v-card-text>
    ...
    methods:{
        async emitData(data){
            ...
            if(this.selectedInbox.facilityId !== null){
                this.selectedFacility = await AdminAPI.getFacilityById(this.selectedInbox.facilityId);
            }
            else{
                this.selectedFacility = null;
            }
            if(this.selectedInbox.status === "accepted" || this.selectedInbox.status === "rejected"){
                this.toggleBtn = true;
            }
            else{
