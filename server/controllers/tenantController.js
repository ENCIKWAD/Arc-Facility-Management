...
module.exports = class TenantController{
  ...
  static async createRequest(req, res) {
    try {
        ...
        const existingRequest = await Request.findOne({ tenantId: request.tenantId, facilityId: facilityId });
        if (existingRequest) {
            return res.status(400).json({ message: "You have already made this request" });
        }
        ...
        if (isLeased) {
            return res.status(400).json({ message: "This facility is already leased within this time" });
        }
